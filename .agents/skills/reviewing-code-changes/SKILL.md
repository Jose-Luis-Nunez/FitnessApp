...
| Centered muscle artwork with clipped category crop + glow | `CategoryTileArtworkStage` |
...
### 15. Visual Regression Coverage (after public-View changes in FitnessUI / FitnessPersistenceUI)
...
struct <ViewName>SnapshotTests {
    @Test func default() {
        let view = <ViewName>(...)
        assertSnapshot(of: view, named: "default", size: CGSize(width: 100, height: 100))
    }
}
```
...
git diff --name-only HEAD -- 'Packages/*/Sources/**/*.swift' | while read f; do
  git diff HEAD -- "$f" | grep -q '^[+-].*public struct.*View\|^[+-].*body' && echo "$f"
done

# Are existing snapshots updated in the same diff?
