...
///
/// 1. **Open with the plan.** The current live schema is `SchemaV6`; opening with
///    `AppMigrationPlan` runs any pending forward migration (V1→V2→V3→V4→V5→V6). This is
...
///    which now finds a valid starting point and runs the full V1→V2→V3→V4→V5→V6 chain.
///
...
///    directory and open a fresh V6 container. We never silently delete user
...
public enum ModelContainerBootstrap {
    ...
    static func makeContainer(storeURL: URL) -> ModelContainer {
        ...
        let liveSchema = Schema(versionedSchema: SchemaV6.self)
