...
/// Selection glass capsule used inside the bar to maintain the same material look
struct LiquidGlassSelection: View {
    let height: CGFloat
    ...
    var body: some View {
        ...
        shape
            ...
            .overlay(shape.fill(Color.black.opacity(0.12)).blendMode(.multiply))
            .overlay(
                shape
                    .stroke(
                        LinearGradient(colors: [Color.white.opacity(0.22), Color.black.opacity(0.16)], startPoint: .topLeading, endPoint: .bottomTrailing),
                        lineWidth: 1
                    )
            )
            .overlay(
                Circle()
                    .fill(RadialGradient(colors: [Color.white.opacity(0.12), .clear], center: .center, startRadius: 2, endRadius: height))
                    .frame(width: height * 0.85, height: height * 0.65)
                    .blur(radius: 14)
                    .opacity(0.45)
                    .offset(x: height * 0.28, y: -height * 0.42)
            )
...
extension View {
    func liquidGlassContainer(cornerRadius: CGFloat, material: Material = .ultraThinMaterial, tintOpacity: Double = 0.28) -> some View {
        background(
            LiquidGlassBackground(cornerRadius: cornerRadius, material: material, tintOpacity: tintOpacity)
        )
    }
}
