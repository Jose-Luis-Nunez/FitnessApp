...
| `BVGSBahnService` | `Packages/FitnessProfile/Sources/FitnessProfile/BVGSBahnService.swift` | — | per-use | **Slim-Orchestrator** (~80 Zeilen) für die S-Bahn-Smart-Routing-Pipeline. Holt Origin-Departures via `BVGTransitClient`, klassifiziert via `SBahnClassifier`, holt Transfer-Pools (Ostbahnhof + Warschauer in parallel) **nur wenn nötig**, und löst Bridges via `SBahnBridgeResolver` auf. Klassifikator-Tabellen, Stop-IDs, Travel-Times, Bridge-Fenster sind in `SBahnRouteConfiguration` ausgelagert. Conforms to `BVGSBahnServicing`. |
| `BVGTransitClient` | `Packages/FitnessProfile/Sources/FitnessProfile/BVGTransitClient.swift` | — | per-use | HTTP-Client über `v6.bvg.transport.rest/stops/{id}/departures?suburban=true`. Returns `[TransitDeparture]` (Domain-Type) statt roher DTOs. Error-Mapping (429/500/decoding/network) identisch zu `BVGTramService`. Conforms to `BVGTransitClienting` für Mock-Injection in `BVGSBahnService`-Integration-Tests. |
| `SBahnClassifier` | `Packages/FitnessProfile/Sources/FitnessProfile/SBahnClassifier.swift` | — | pure namespace | Pure Klassifikations-Funktionen: `classify(line:direction:configuration:)` → `SBahnClassification` (eastDirect / eastShortOstbahnhof / eastShortWarschauer / west / unknown), und `isEastDirectAtTransfer(...)` für Bridge-Pool-Filter. Tabellen-driven über die übergebene `SBahnRouteConfiguration`. Stateless, ohne I/O — direkt mit `#expect(...)` testbar. |
| `SBahnBridgeResolver` | `Packages/FitnessProfile/Sources/FitnessProfile/SBahnBridgeResolver.swift` | — | pure namespace | Pure Logik: für eine Short-Turn-Departure einen eigenständigen Bridge-Trip im 5-min-Fenster nach Ankunft am Wechsel-Bahnhof finden. Eigenständigkeit per `tripId ∉ alexTripIds`. Falls beide Wechselpunkte (Ostbahnhof + Warschauer) Kandidaten haben → frühester Bridge-Departure gewinnt. Stateless, deterministisch. |
| `SBahnRouteConfiguration` | `Packages/FitnessProfile/Sources/FitnessProfile/SBahnRouteConfiguration.swift` | — | data | Immutable Wertobjekt mit allen tunbaren Routing-Werten (Stop-IDs, Travel-Times, Bridge-Fenster, Klassifikator-Tabellen für east-via-Ostkreuz / east-bypass-S9 / westbound). Default-Statisch `.standardBerlin` für die Alex → Ostkreuz Stadtbahn. Tests können alternative Konfigurationen injizieren. |
