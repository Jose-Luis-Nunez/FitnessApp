...
Packages/
  FitnessProfile/     — SPM library for profile feature (`BMIService`, `ProfileViewModel`, `ProfileStore`, `BVGTramService`, `TramDeparturesViewModel`, `TramDeparturesCardView`, `TramDeparturesCache`, `BVGSBahnService` (slim orchestrator), `BVGTransitClient` (HTTP-Layer), `SBahnRouteConfiguration` (injektierbare Stop-IDs/Travel-Times/Klassifikator-Tabellen), `SBahnClassifier` (pure Klassifikations-Funktionen), `SBahnBridgeResolver` (pure Bridge-Resolution), `SBahnDeparturesViewModel`, `SBahnDeparturesCardView`, `SBahnDeparturesCache`, `SBahnDeparture` + `BridgeHint` + `TransitDeparture` domain types). Depends on `FitnessUI`. Tests: `BMIServiceTests` (stubbed API), `ProfileViewModelTests`, `BVGTramServiceTests` (stubbed API), `TramDeparturesViewModelTests` (MockService + MockCache), `TramDeparturesCacheTests` (ephemerer `UserDefaults(suiteName:)`), `SBahnClassifierTests` (pure table-driven Tests pro Linie/Direction-Kombination, keine Mocks), `SBahnBridgeResolverTests` (pure mit synthetischen `[TransitDeparture]`-Pools, Eigenständigkeits-Filter, 5-min-Window-Boundary, Earliest-Wins), `BVGTransitClientTests` (HTTP-Error-Mapping mit `StubURLProtocol`), `BVGSBahnServiceTests` (Integration mit MockClient + echten Pure-Komponenten: Happy-Path, Bridge-Resolution, Pass-Through-Filter, Sortierung+Clipping), `SBahnDeparturesViewModelTests` (MockService + MockCache). **Tram-Sub-Feature**: aufklappbare Karte mit Start↔Destination-Swap, nächste 3 Tram-21-Abfahrten zwischen Blockdammweg (`900162504`) und Marktstr. (`900160535`) aus `v6.bvg.transport.rest/stops/{id}/departures?tram=true`. **S-Bahn-Sub-Feature** (direkt unter Tram-Karte gerendert, gleiche Aufklapp-Mechanik): Strecke S+U Alexanderplatz (`900100003`) ↔ S Ostkreuz (`900120003`), nächste 6 S-Bahn-Abfahrten. Klassifikator unterscheidet `east-direct` (S3/S5/S7 die durch Ostkreuz fahren), `east-short@Ostbahnhof` (Wendezüge), `east-short@Warschauer` (S9-Trips Richtung BER, plus echte Warschauer-Endstationen). Bei `east-short` wird per Bridge-Resolver geprüft, ob am Wechsel-Bahnhof (Ostbahnhof oder Warschauer Str., parallel beide getestet) eine **eigenständige** Anschluss-S-Bahn fährt — Eigenständigkeit per `tripId`-Vergleich gegen Alex-Pool: matching tripId = Pass-Through-Bahn (= "S-Bahn die der User auch beim Warten am Alex erwischen würde") = nicht als Bridge zugelassen. Trips ohne useful Bridge werden ausgeblendet. Pro Row klickbar zur Detail-Ansicht (geplante vs aktuelle Abfahrt, Verspätung, Endstation, ggf. Umstiegs-Details). **Refresh-Strategie identisch zu Tram**: einmaliger Refresh beim Aufklappen, manueller Refresh via `RefreshActionButton`, `onBecameActive()` triggert nur wenn `lastUpdated > 60 s`. UserDefaults-Cache (`SBahnDeparturesCache`, Key `sbahn.cache.<from>.<to>`) als Offline-Fallback mit `isStale=true`-Flag. BMI- und Körperdaten-Karten in `ProfileView` nutzen dasselbe Aufklapp-Pattern.
...
Tests/
  ...
  Packages/*/Tests/       — Package-level unit tests per SPM module
    ...
    FitnessUITests/          — Snapshot tests (swift-snapshot-testing, 26 tests): CardBackgroundSnapshotTests (gradient/glass/no-padding/idle), CardShellSnapshotTests (idle-theme/completed-with-edge-indicator/no-trailing/with-expanded-content), MiniActionMenuSnapshotTests (with-title/no-title/confirm-delete), WorkoutDropdownSnapshotTests (collapsed/expanded/long-name), SetTileViewSnapshotTests (with-weight/bodyweight), ProgressBarSnapshotTests (empty/partial/full), MetricChipViewSnapshotTests (default/wide), CapsuleToggleStyleSnapshotTests (on/off), IdlePlayButtonSnapshotTests (idle), RefreshActionButtonSnapshotTests (ready/loading). Reference PNGs in `__Snapshots__/`.
...
| `ExerciseManagementService` | `Packages/FitnessStorage/.../ExerciseManagementService.swift` | `\.exerciseManagement` | singleton | Exercise business logic (add, remove, reorder). Conforms to `ExerciseManaging` protocol (`FitnessCore`). Constructor-DI for `ExerciseStoring`, `AnalyticsStoring`, `WorkoutStoring` (defaults to `Container.shared`). |
...
| `BVGSBahnService` | `Packages/FitnessProfile/Sources/FitnessProfile/BVGSBahnService.swift` | — | per-use | **Slim-Orchestrator** (~120 Zeilen) für die S-Bahn-Smart-Routing-Pipeline. Hält ein Array `routeConfigurations` (Default: `[.standardBerlinForward, .standardBerlinReverse]`). Pro `fetchSBahnRoute(from:to:max:)` matcht (from, to) gegen die Configurations. **Match** → Full-Pipeline (Klassifikator + Bridge-Resolver), **No Match** → Fallback `fetchDirectionFiltered` mit server-seitigem `direction=`-Filter (route-aware ohne lokale Tabellen). Bridges für die konfigurierten Routen funktionieren symmetrisch in beide Richtungen — z.B. Reverse Ostkreuz → Alex erkennt Short-Turns @ Ostbahnhof / @ Warschauer mit eigenständigen West-bound Bridges. Conforms to `BVGSBahnServicing`. |
| `BVGTransitClient` | `Packages/FitnessProfile/Sources/FitnessProfile/BVGTransitClient.swift` | — | per-use | HTTP-Client über `v6.bvg.transport.rest/stops/{id}/departures?suburban=true`. `fetchSuburbanDepartures(stopId:directionStopId:)` mit optionalem `direction`-Filter (server-seitig route-aware). Returns `[TransitDeparture]` (Domain-Type) statt roher DTOs. Error-Mapping (429/500/decoding/network) identisch zu `BVGTramService`. Conforms to `BVGTransitClienting` für Mock-Injection in Integration-Tests. |
| `SBahnClassifier` | `Packages/FitnessProfile/Sources/FitnessProfile/SBahnClassifier.swift` | — | pure namespace | Pure Klassifikations-Funktionen: `classify(line:direction:configuration:)` → `SBahnClassification` (eastDirect / eastShortOstbahnhof / eastShortWarschauer / west / unknown), und `isEastDirectAtTransfer(...)` für Bridge-Pool-Filter. **Direction-neutral:** ließt `passesDestinationKeywords` / `wrongDirectionKeywords` / `bypassLineKeywords[line]` aus der Konfiguration; was "passes" oder "wrong" heißt hängt von der jeweils gemounted Forward/Reverse-Configuration ab. Stateless, ohne I/O — direkt mit `#expect(...)` testbar. |
| `SBahnBridgeResolver` | `Packages/FitnessProfile/Sources/FitnessProfile/SBahnBridgeResolver.swift` | — | pure namespace | Pure Logik: für eine Short-Turn-Departure einen eigenständigen Bridge-Trip im 5-min-Fenster nach Ankunft am Wechsel-Bahnhof finden. Eigenständigkeit per `tripId ∉ originTripIds`. Falls beide Wechselpunkte (Ostbahnhof + Warschauer) Kandidaten haben → frühester Bridge-Departure gewinnt. Stateless, deterministisch. Funktioniert symmetrisch in Forward/Reverse durch direction-neutrale Konfiguration. |
| `SBahnRouteConfiguration` | `Packages/FitnessProfile/Sources/FitnessProfile/SBahnRouteConfiguration.swift` | — | data | Immutable Wertobjekt mit allen tunbaren Routing-Werten pro (origin → destination)-Pair. Direction-neutral benannte Felder: `passesDestinationKeywords` (Endstationen die das Ziel überfahren), `wrongDirectionKeywords` (filtern), `bypassLineKeywords: [String: [String]]` (Line → Bypass-Endstationen, z.B. S9 → BER für Forward), `transferStopIds`, `travelTimes` (richtungsspezifisch — Alex→Warschauer = 6min vs Ostkreuz→Warschauer = 2min), `bridgeWindow`. Statische Defaults: `.standardBerlinForward` (Alex → Ostkreuz mit S9-Bypass) und `.standardBerlinReverse` (Ostkreuz → Alex ohne Bypass). `.standardBerlin` als Backward-compat Alias = forward. |
| `SBahnDeparturesCache` | `Packages/FitnessProfile/Sources/FitnessProfile/SBahnDeparturesCache.swift` | — | per-use | Persistiert die letzte erfolgreiche `[SBahnDeparture]`-Antwort (inkl. `BridgeHint`-Daten) pro `(from, to)` in `UserDefaults` (Key `sbahn.cache.<from>.<to>`, JSON via `.iso8601`). Selber Fallback-Pfad wie `TramDeparturesCache`. Conforms to `SBahnDeparturesCaching`. |
...
| `CardShell` | `Packages/FitnessUI/Sources/FitnessUI/CardShell.swift` | Structural card wrapper standardizing header layout (leading icon, title, trailing action), expanded content slot, optional `contentBackground` overlay, edge indicator, shadow, and padding. Built on `CardBackground` + `CardTheme`. Used by `IdleActiveCardModelView` and `InactiveCardModelView`. Cards that don't fit the slot pattern (e.g. `ActiveCardModelView`) use `CardBackground` directly. |
...
| `MetricColumnView` | `Packages/FitnessUI/Sources/FitnessUI/MetricColumnView.swift` | Labeled metric column for card metric rows — label aligned to `.metricLabel` above caller-provided content with optional tap gesture. Used by `IdleActiveCardModelView` for Weight and Seat columns. |
