...
| `LevelIncrease` | A derived weight or rep increase: the session that first reached a level, paired with the last session before it. Only steps up are modelled; the type was named `WeightPhase` while it still modelled a period at one weight. |
...
| Tile rows | `TileScrollRow` owns the horizontal tile strip used by both exercise cards' set rows and the idle card's coaching row: the caller declares the trailing accessory's reserved width and an explicit row height, because the width arithmetic lives in a `GeometryReader`. A row that acts as one control passes a `TileRowControl` (action, hint and identifier together); a row whose tiles each have their own destination passes none and attaches gestures per tile. Nothing may be layered over the scroll view — an overlaid control becomes the topmost hit-test target and swallows the horizontal pan. `TileScrollRowMetrics` holds the arithmetic so it is testable without rendering. |
