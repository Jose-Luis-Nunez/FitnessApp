...
For shared conventions (DSL, selectors, layout, **diagnosis decision tree**) see [ui-test-conventions.md](../../references/ui-test-conventions.md).
...
See `.cursor/rules/build-and-test.mdc` § "UI Tests" for the exact `xcodebuild` command. Run a **single failing test** (`-only-testing:`) instead of the full suite while debugging.

### Step 2 — Diagnose the Selector

Work the **5-step decision tree** in [ui-test-conventions.md § Diagnosing a Failing Selector](../../references/ui-test-conventions.md#diagnosing-a-failing-selector) **strictly in order**:
...
Re-run the single test, confirm green, then update `.cursor/hooks/state/test-execution.stamp.md` with the result so the stop-hook gate passes.
