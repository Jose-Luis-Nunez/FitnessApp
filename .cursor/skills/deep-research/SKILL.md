...
description: >-
  ...
  triangulation, and citation-backed reports. Use when the user asks for deep
  research, comprehensive analysis, technology comparison, trend analysis, state
  of the art review, or architecture evaluation. Not for simple lookups,
  debugging, or questions answerable with 1-2 searches.
---
...
A single Cursor agent has ~25 tool calls per interaction — far less than
ChatGPT Deep Research or Claude Research. **Subagents bypass this limit:**
each subagent (via the `Task` tool) gets its own 25-call budget and isolated
context. Cursor supports up to 4 subagents running in parallel.
