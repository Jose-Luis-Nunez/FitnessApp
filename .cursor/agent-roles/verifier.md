...
You receive:
- A list of changed `.cursor/` files
- A validation report from the main agent

## Verification Steps — do ALL of these

1. **Reference Integrity:** Run `rg` for any old/stale names mentioned in the report. Also run `rg` for common stale patterns across `.cursor/` files.
2. **Overview Sync:** List actual files in `.cursor/rules/`, `.cursor/skills/*/`, `.cursor/hooks/checks/`. Compare with tables in `.cursor/references/agent-system-overview.md`.
...
If all checks pass (or the report correctly identified all issues), write to `.cursor/hooks/state/agent-infrastructure.stamp.md`:
