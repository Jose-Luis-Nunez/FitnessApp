...
Packages/
  ...
  FitnessTestSupport/ — Shared test utilities: `makeExercise` factory, `MockAnalyticsStorage`, `StubAnalyticsStorage`, `MockExerciseStorage`, `MockWorkoutStorage` (mutates state on delete/rename/duplicate), `MockTotalAnalyticsStorage`, `MockExerciseManagement` (full `ExerciseManaging` impl with spy fields `updatedExercises`/`resetExercises`), `InMemoryFeedbackStorage` (upsert-by-sessionId `FeedbackStoring`), `waitUntil` with timeout assertion. Depends on `FitnessCore` + `Testing`. **Mockable adoption blocker**: `swift-testing` (explicit SPM dep) conflicts with `swift-syntax` (transitive from any macro library like Mockable/swift-snapshot-testing) via `_TestingInternals` module clash. Prerequisite: remove explicit `swift-testing` from all `Package.swift` and rely on Xcode-bundled `Testing` framework.
...
Tests/
  ...
  Packages/*/Tests/       — Package-level unit tests per SPM module
    ...
    FitnessUITests/          — Snapshot tests (swift-snapshot-testing): CardBackgroundSnapshotTests (gradient/glass/no-padding), MiniActionMenuSnapshotTests (with-title/no-title/confirm-delete), WorkoutDropdownSnapshotTests (collapsed/expanded/long-name). Reference PNGs in `__Snapshots__/`.
