...
Tests/
  ...
  Packages/*/Tests/       — Package-level unit tests per SPM module
    ...
    FitnessExerciseTests/   — MuscleCategorySelectionViewModelTests (categories, exercise counts, card VM cache, reset, find category, exercise mutations, coordinator completion integration, exercise stability, **T7a `currentWorkoutId` exposure**), MuscleCategoryViewModelTests (cardViewModel cache, mutations, refresh-on-coordinator-completion, **T7b `currentWorkoutId` exposure**), ExerciseFormViewModelTests, ResetAllExercisesUseCaseTests
