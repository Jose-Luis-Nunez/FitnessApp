...
Features/
  ...
  Exercise/
    ...
    MuscleCategory/   — Muscle category detail screen with exercises. **T7b**: `MuscleCategoryView.exerciseListSection` now renders `ExerciseCardModelView` from `FitnessPersistenceUI` driven by a local `@Query<ExerciseModel>` filtered on `(workoutId, category)` — Bug 1 fixed live because the variant resolves from `model.isCompleted` instantly after `coordinator.finishExercise()` writes it. View identity is rebound on workout switch via `.id(viewModel.currentWorkoutId)`. The legacy `viewModel.exercises` snapshot stays alive only for the edit/picker/menu condition flags (`hasActiveExercise`, `hasCompletedExercises`, `add`, `updateExercise`); T8 deletes that path together with the `changeVersion` observation loop.
