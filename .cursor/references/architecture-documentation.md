...
Packages/
  ...
  FitnessTestSupport/ — Shared test utilities: `makeExercise` factory, `MockAnalyticsStorage`, `StubAnalyticsStorage`, `MockExerciseStorage`, `MockWorkoutStorage` (mutates state on delete/rename/duplicate), `MockTotalAnalyticsStorage`, `MockExerciseManagement` (full `ExerciseManaging` impl with spy fields `updatedExercises`/`resetExercises`), `InMemoryFeedbackStorage` (upsert-by-sessionId `FeedbackStoring`), `waitUntil` (throws `WaitUntilTimeoutError` on timeout). Depends on `FitnessCore` only. No explicit `swift-testing` SPM dependency — `import Testing` resolves from the Xcode toolchain (Xcode 16+). This was cleaned up to eliminate the `_TestingInternals` module conflict that blocked adoption of macro-based libraries (Mockable, swift-snapshot-testing, etc.).
