...
| [0002](../../docs/adr/0002-persistence-ui-package.md) | FitnessPersistenceUI Package | accepted (T4 skeleton + T5 cards + T6 tile + T7-0 cycle-break + T7a tile rollout + T7b card rollout landed) |
...
Features/
  ...
  MuscleGroupSelection/ — Home screen: muscle group category grid. MuscleCategorySelectionViewModel accepts optional `coordinatorCache`, `exerciseManagement`, `workoutStorage`, and `exerciseStorage` via constructor injection (defaults to Factory singleton). Observes `exerciseStorage.changeVersion` to reactively refetch exercises from storage. **T7a**: the overview-mode tile grid (`MuscleCategorySelectionView.categoryList`) now renders `CategoryTileModelView` from `FitnessPersistenceUI` (gated on `viewModel.currentWorkoutId`) — Bug 2 fixed live because the tile's `@Query<ExerciseModel>` reacts to SwiftData writes without going through `refreshExercises()`. The list-mode (`allExercisesList` → `ExerciseCardContainerView`) is untouched and is migrated by T7b.
